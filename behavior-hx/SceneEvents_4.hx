...
class SceneEvents_4 extends SceneScript
{
	...
	override public function init()
	{
		...
		playSoundOnChannel(getSound(52), Std.int(3));
		...
		addSceneCollisionListener(getActorType(2).ID, getActorType(6).ID, function(event:Collision, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				switchScene(GameModel.get().scenes.get(5).getID(), createPixelizeOut(1, Utils.getColorRGB(102,0,153)), createFadeIn(1, Utils.getColorRGB(0,0,0)));
			}
		});
		...
		addWhenTypeGroupKilledListener(getActorType(2), function(eventActor:Actor, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				switchScene(GameModel.get().scenes.get(4).getID(), createFadeOut(2, Utils.getColorRGB(153,0,153)), createFadeIn(2, Utils.getColorRGB(0,0,0)));
			}
		});
