...
class Design_31_31_FireBulletWave extends ActorScript
{
	...
	public var _FirstDirection:Float;
	...
	public function new(dummy:Int, actor:Actor, dummy2:Engine)
	{
		...
		_NumberofBullets = 3.0;
		nameMap.set("First Direction", "_FirstDirection");
		_FirstDirection = 0.0;
				...
				if((hasValue(_BulletType)))
				{
					...
					if(_Fire)
					{
						...
						if((!(_Wait) && (!(_UseAmmunition) || (_CurrentAmmunition > 0))))
						{
							...
							_FirstDirection = asNumber((_FireDirection - (((_NumberofBullets - 1) / 2) * _BulletSpread)));
							propertyChanged("_FirstDirection", _FirstDirection);
							for(index0 in 0...Std.int(_NumberofBullets))
							{
								createRecycledActor(_BulletType, 0, 0, Script.FRONT);
								_FireDirection = asNumber((_FirstDirection + (index0 * _BulletSpread)));
								propertyChanged("_FireDirection", _FireDirection);
								if(_UseAmmunition)
								{
								...
								actor.setActorValue("FireBullet_Creator", actor);
							}
						}
					}
				}
